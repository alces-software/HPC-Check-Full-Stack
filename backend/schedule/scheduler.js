...
class Scheduler {
   ...
   async dateToNowsi(db, date) {
      ...
      const closedDocs = await closedColl.find({ day: { $gte: start, $lt: end } }).toArray();
   ...
   async getClosedSet(db, startDate) {
      ...
      return new Set(closedDocs.map((d) => new Date(d.day).toISOString().slice(0, 10)));
   }
   ...
   isClosed(date, closedSet) {
      const isoDay = date instanceof Date ? date.toISOString().slice(0, 10) : date;
   ...
   async getPersonFromIndex(db, index) {
      const doc = await db
         ...
         .findOne({ personNum: index }, { projection: { personId: 1 } });
