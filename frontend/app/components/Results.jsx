   ...
   useEffect(() => {
      async function loadReports() {
         ...
         try {
            ...
            if (mode === "week") {
               url = `${process.env.NEXT_PUBLIC_API_URL}/report/week?page=${page}&limit=20`;
            } else {
               url = `${process.env.NEXT_PUBLIC_API_URL}/report/cluster/${selectedClusterId}?page=${page}&limit=20`;
            }
               ...
               <div className={mode === "week" ? "opacity-40 pointer-events-none" : ""}>
                  <Listbox value={selectedClusterId} onChange={setSelectedClusterId}>
                     <div className="relative">
                        ...
                        <ListboxOptions
                           className="absolute z-50 mt-2 max-h-60 w-full overflow-auto rounded-xl border border-white/10 bg-slate-900/95 backdrop-blur-xl shadow-2xl"
                        >
                           {clusters.map((c) => (
                              <ListboxOption
                                 key={c.id}
