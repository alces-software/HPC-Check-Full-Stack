...
export default function ClusterInstructionsPDF({ cluster, clusterId, steps }) {
   return (
      <Document>
         <Page size="A4" style={styles.page} wrap>
            <View style={styles.header}>
               <Text style={styles.title}>{cluster.name} Instructions</Text>

               <Text style={styles.subtitle}>Cluster ID: {clusterId}</Text>

               <Text style={styles.subtitle}>Daily cluster checks and methods</Text>
            </View>
            ...
            {steps.map((step, index) => (
               <View key={step.id} style={styles.stepCard} wrap={false}>
                  <View style={styles.stepHeader}>
                     ...
                     {step.expectedTime && (
                        <Text style={styles.expectedTime}>{step.expectedTime}</Text>
                     )}
                  </View>
                  ...
                  {(step.methods || []).length === 0 ? (
                     <Text style={styles.description}>No methods available.</Text>
                  ) : (
                     (step.methods || []).map((method, methodIndex) => (
                        <View key={method.id} style={styles.methodItem}>
                           <Text style={styles.methodNumber}>{methodIndex + 1}</Text>
                     ...
                     <View style={styles.outcomeGrid}>
                        {step.good && (
                           <View style={styles.goodBox}>
                              ...
                              <Text style={styles.outcomeText}>{step.good}</Text>
                           </View>
                        )}
                        ...
                        {step.bad && (
                           <View style={styles.badBox}>
                              ...
                              <Text style={styles.outcomeText}>{step.bad}</Text>
                           </View>
                        )}
                     </View>
                  )}
            ...
            <Text style={styles.footer}>Generated from cluster settings</Text>
         </Page>
      </Document>
   );
}
